...
import app.src.dao.investor_dao as investordao
...
@investor_bp.route('/get-all-investors', methods=['GET'])
def get_all():
    try:
...
@investor_bp.route('/get-investors-by-name', methods=['GET'])
def get_investors_by_name(): 
    try:
        investors = []
        name = request.args.get('name')
        investors = investordao.get_all_investors() if name is None else investordao.get_investors_by_name(name)
        res = make_response()
        res.response = json.dumps([investor.__dict__ for investor in investors])
        res.headers['Content-Type'] = 'application/json'
        res.status = HTTPStatus.OK
...
def update_investor_address(id, new_addr):
    try:
...
def update_investor_name(id, new_name):
    try:
...
@investor_bp.route('/update-status/<id>/<new_status>', methods=['PUT'])
def update_investor_status(id, new_status):
    try:
        investordao.update_investor_status(id, new_status)
        res = make_response()
        res.status = HTTPStatus.OK
        return res
    except Exception as e:
        ...
        error_res.status = HTTPStatus.INTERNAL_SERVER_ERROR 
        error_res.headers['Content-Type'] = 'plain/text'
        error_res.response = f'Error while updating investor (ID: {id}) status: {str(e)}'
        return error_res
